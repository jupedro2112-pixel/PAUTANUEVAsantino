...
- [x] D — SG Redis: el nodo usa el SG default `sg-05821d7d9b52c9ecf`, regla 6379 desde el SG de instancias `sg-0c7bd403723c54794` ✅ · [x] deploy `v20260908-1919` (por CLI desde CloudShell: `git archive` → S3 bucket `elasticbeanstalk-sa-east-1-062472745735` → create-application-version → update-environment) · [x] app arriba: `curl` 200 en la URL EB (2026-09-08 19:24 UTC) · [ ] verificación panel/log (SSM 33, Redis adapter) · [ ] dominio **auto1girox.com** (decidido 2026-09-08): cert ACM con SANs www + **panel** (el admin va en `panel.auto1girox.com`, DNS-only) → listener 443 por `update-environment` → CNAME @/www/panel en Cloudflare · [ ] SSM PUBLIC_BASE_URL=https://auto1girox.com, ALLOWED_ORIGINS (+www +panel), ADMIN_HOST=panel.auto1girox.com + restart. ⚠️ El panel NO loguea por HTTP: cookies `Secure` en producción (server.js ~L1103) · [ ] hgcash webhook · [ ] Firebase authorized domain
